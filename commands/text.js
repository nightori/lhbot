...
const fancy = require('./../static/fancyText.json');
...
module.exports = {
	names: ['text', 'style'],
	description: 'Вывести заданный текст в разных стилях',
	args: null,
	restricted: false,
	serverOnly: false,
	hidden: false,
	execute(msg) {
		...
		const embed = new Discord.MessageEmbed().setColor(cfg.embedColor);
		...
		for (let i = 0; i < styleNames.length; i++) {
			...
			for (const letter of text) {
				...
				if (styleNames[i] == 'Full width' && styledLetter == ' ') styledLetter = '\u3000';
